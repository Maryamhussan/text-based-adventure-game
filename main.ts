...
let enemies: string[] = ["Monster", "Dragon", "Zombie", "Snake"];
let maxEnemyHealth: number = 100;
let enemyAttackDamage: number = 25;
let maxPlayerHealth: number = 100;
let minPlayerHealth: number = 0;
let playerAttackDamage: number = 30;
let healthPotions: number = 3;
let healingAmount: number = 30;
let potionChance: number = 50;
let enemiesDefeated: number = 0;

let randomNumber = (max: number, min: number = 0): number => {
  ...
  if (playerName) {
    ...
    if (start.confirmStart) {
      GAME: while (true) {
        ...
        let enemy: string = enemies[randomNumber(enemies.length - 1)];
        console.log("\t=======================================");
        ...
        while (enemyHealth > 0) {
          ...
          switch (userInput.input) {
            ...
            case "\tAttack":
              let attackDamage: number = randomNumber(playerAttackDamage, 1);
              let damageTaken: number = randomNumber(enemyAttackDamage, 1);
